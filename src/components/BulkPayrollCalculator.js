...
import { Users, Plus, Trash2, Calculator, Download, Eye, EyeOff, Upload, FileSpreadsheet, Printer, AlertTriangle, Building2 } from 'lucide-react';
...
const BulkPayrollCalculator = ({ 
  ...
  const downloadExcelTemplate = () => {
    try {
      ...
      // Headers row (updated with new fields)
      const headers = [
        'Employee Name*', 'TIN', 'Basic Salary*', 'Transport Allowance', 'Housing Allowance',
        'Meal Allowance', 'Utility Allowance', 'Medical Allowance', 'Other Allowances', 
      ...
      // Sample data (with new fields: Utility, Medical, Mortgage Interest)
      const sampleData = [
        ['John Doe', '12345678', 500000, 50000, 200000, 30000, 20000, 25000, 25000, '', '', 10000, 15000, 5000, 1200000, 500000],
        ['Jane Smith', '87654321', 400000, 40000, 150000, 25000, 15000, 20000, 20000, '', '', 8000, 12000, 4000, 1000000, 0],
        ['Mike Johnson', '11223344', 600000, 60000, 250000, 35000, 25000, 30000, 30000, '', '', 12000, 18000, 6000, 1500000, 750000],
    ...
    reader.onload = (e) => {
      try {
        ...
        if (file.name.endsWith('.xlsx') || file.name.endsWith('.xls')) {
          ...
          // Convert to employee objects (updated with new fields)
          const uploadedEmployees = dataRows.map((row, index) => ({
            ...
            pension_contribution: row[9] ? row[9].toString().trim() : '',
            nhf_contribution: row[10] ? row[10].toString().trim() : '',
            life_insurance_premium: row[11] ? row[11].toString().trim() : '',
            health_insurance_premium: row[12] ? row[12].toString().trim() : '',
            nhis_contribution: row[13] ? row[13].toString().trim() : '',
            annual_rent: row[14] ? row[14].toString().trim() : '',
            mortgage_interest: row[15] ? row[15].toString().trim() : '',
            calculated: false,
            result: null
          ...
          const uploadedEmployees = dataRows.map((row, index) => ({
            ...
            pension_contribution: row[9] || '',
            nhf_contribution: row[10] || '',
            life_insurance_premium: row[11] || '',
            health_insurance_premium: row[12] || '',
            nhis_contribution: row[13] || '',
            annual_rent: row[14] || '',
