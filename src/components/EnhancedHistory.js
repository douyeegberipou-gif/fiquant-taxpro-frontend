...
const EnhancedHistory = ({ history, citHistory, formatCurrency, hasFeature, historyLocked = false, onUnlockHistory }) => {
  ...
  const handleExportSavedToExcel = () => {
    ...
    try {
      ...
      const summaryData = savedCalculations.map(calc => ({
        'Name': calc.calculation_name || 'Unnamed',
      ...
      calcTypes.forEach(type => {
        ...
        if (typeCalcs.length > 0) {
          const typeData = typeCalcs.map(calc => {
            ...
            return { 'Name': calc.calculation_name || 'Unnamed' };
          });
  ...
  const handleExportSingleToExcel = (savedCalc) => {
    ...
    try {
      ...
      if (calcType === 'paye') {
        detailData = [
          { 'Field': 'Calculation Name', 'Value': savedCalc.calculation_name || 'Unnamed' },
      ...
      const fileName = `${savedCalc.calculation_name || 'Calculation'}_${calcType?.toUpperCase()}_${new Date().toISOString().split('T')[0]}.xlsx`;
      XLSX.writeFile(wb, fileName);
    } catch (error) {
      console.error('Error exporting to Excel:', error);
      alert('Failed to export to Excel');
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              <h3 className="font-semibold text-gray-900">
                                {savedCalc.calculation_name || 'Unnamed Calculation'}
                              </h3>
