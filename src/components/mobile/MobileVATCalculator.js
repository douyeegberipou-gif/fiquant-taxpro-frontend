...
import { Receipt, Calculator, DollarSign, FileText, RotateCcw, ChevronDown, ChevronUp, Building2, Plus, Trash2 } from 'lucide-react';
...
export const MobileVATCalculator = () => {
  ...
  const calculateVAT = async () => {
    ...
    try {
      ...
      let standardTotal = 0, exemptTotal = 0, zeroRatedTotal = 0;
      const transactionBreakdown = [];

      validTransactions.forEach(t => {
        const config = transactionTypes[t.transaction_type];
        const amount = parseFloat(t.amount) || 0;
        const vatAmount = config.category === 'standard' ? amount * 0.075 : 0;
        ...
        if (config.category === 'standard') standardTotal += amount;
        else if (config.category === 'exempt') exemptTotal += amount;
        else zeroRatedTotal += amount;
      });

      const payload = {
        company_name: vatInput.company_name,
        tin: vatInput.tin,
        tax_authority: vatInput.tax_authority,
        month: vatInput.month,
        standard_rated_sales: standardTotal,
        vat_exempt_sales: exemptTotal,
        zero_rated_sales: zeroRatedTotal,
        total_sales: standardTotal + exemptTotal + zeroRatedTotal,
        is_registered_business: vatInput.is_registered_business,
        transactions: validTransactions.map(t => ({
          ...t,
          amount: parseFloat(t.amount) || 0,
          category: transactionTypes[t.transaction_type]?.category
        }))
      };
