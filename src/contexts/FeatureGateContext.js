...
export const FeatureGateProvider = ({ children }) => {
  ...
  const requiresUpgrade = (feature) => {
    ...
    const upgradeRequirements = {
      ...
      'cit_calc': tier === 'free' ? 'pro' : null,
      'vat_calc': tier === 'free' ? 'pro' : null,
      'cgt_calc': tier === 'free' ? 'pro' : null,
  ...
  const getFeatureMessage = (feature) => {
    ...
    const messages = {
      ...
      'cit_calc': `CIT calculator requires ${upgradeRequired.charAt(0).toUpperCase() + upgradeRequired.slice(1)} tier or higher.`,
      'vat_calc': `VAT calculator requires ${upgradeRequired.charAt(0).toUpperCase() + upgradeRequired.slice(1)} tier or higher.`,
      'cgt_calc': `CGT calculator requires ${upgradeRequired.charAt(0).toUpperCase() + upgradeRequired.slice(1)} tier or higher.`,
