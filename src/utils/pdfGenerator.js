...
export const generateCITReport = (citInput, result) => {
  ...
  // ===== TWO-COLUMN COMPUTATION LAYOUT =====
  // Redefine column positions for the computation sections below the line
  const compLeftColStart = margin;
  const compRightColStart = pageWidth / 2 + 3;
  const computationStartY = yPos;
  let leftY = computationStartY;
  let rightY = computationStartY;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(59, 130, 246);
  doc.text('A. INCOME & EXPENSES', compLeftColStart, leftY);
  leftY += 4;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(34, 197, 94);
  doc.text('B. RELIEFS & ALLOWANCES', compLeftColStart, leftY);
  leftY += 4;
  ...
  autoTable(doc, {
    body: [
      ...
      ['Total Available CA', formatCurrency(result.total_available_ca || result.total_capital_allowances || 0)],
      ...
      ['Unutilised Losses c/f', formatCurrency(result.unutilised_losses_cf || 0)]
    ],
    startY: leftY,
    theme: 'grid',
    styles: { fontSize: 7, cellPadding: 1.5 },
  ...
  // Section C: WHT Credits (always show)
  doc.setFontSize(8);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(139, 92, 246);
  doc.text('C. WHT CREDITS', compLeftColStart, leftY);
  leftY += 4;
  ...
  // ----- RIGHT COLUMN: D (Tax Computation) -----
  doc.setFontSize(8);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(30, 30, 30);
  doc.text('D. TAX COMPUTATION', compRightColStart, rightY);
  rightY += 4;
  ...
  const adjustedProfit = result.adjusted_profit || result.assessable_profit || 0;
  const assessableProfit = result.assessable_profit || result.taxable_profit || 0;
  const taxableProfit = result.taxable_profit || 0;
  
  autoTable(doc, {
    body: [
      ['Adjusted Profit', formatCurrency(adjustedProfit)],
      ['Less: Loss Relief', `(${formatCurrency(result.carry_forward_losses_applied || 0)})`],
      ['Assessable Profit (Dev. Levy Base)', formatCurrency(assessableProfit)],
      ['Less: Capital Allowances', `(${formatCurrency(result.total_capital_allowances || 0)})`],
      ['Taxable Profit (CIT Base)', formatCurrency(taxableProfit)],
      [`CIT @ ${citRate} on Taxable Profit`, formatCurrency(result.cit_due || 0)],
      [`Dev. Levy @ ${devLevyRate} on Assessable Profit`, formatCurrency(result.development_levy || 0)],
    ...
    tableWidth: 95,
    margin: { left: compRightColStart }
  });
  rightY = doc.lastAutoTable.finalY + 5;
  
  // Net Tax Payable - Highlighted Box
  autoTable(doc, {
    ...
    columnStyles: {
      0: { cellWidth: 60, fillColor: [220, 38, 38], textColor: [255, 255, 255] },
      1: { cellWidth: 35, halign: 'right', fillColor: [254, 226, 226], textColor: [185, 28, 28] }
    },
    tableWidth: 95,
    margin: { left: compRightColStart }
  });
